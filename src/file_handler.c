...
void saveStudent(Student *s) {
    ...
    if (f == NULL) {
        printf("Error: Cannot open student.dat\n");
        return;
    }
...
void updateStudentFile(Student arr[], int n) {
    FILE *f = fopen("data/student.dat", "wb");
    if (f == NULL) {
        printf("Error: Cannot rewrite student.dat\n");
        return;
    }
...
/* Search student by name (partial match) implemented in student.c */